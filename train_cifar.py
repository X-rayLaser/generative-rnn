...
import config
from util import shrink_all, train_model, train_model_mse
...
x_train = x_train[(y_train == 1).ravel()]
...
#model = train_model(x_train, config.num_classes, epochs=100)
model = train_model_mse(x_train, config.num_classes, epochs=500)
